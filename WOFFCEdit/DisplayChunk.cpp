...
//Additional
bool DisplayChunk::GetIntersection(XMVECTOR rayOrigin, XMVECTOR rayDirection, XMVECTOR &intersectPoint)
{
	float closestDistance = 10000;

	//By default no intersection is detected
	bool hitTerrain = false;

	//For each position on heightmap
	for (int x = 0; x < TERRAINRESOLUTION - 1; x++)
	{
		for (int y = 0; y < TERRAINRESOLUTION - 1; y++)
		{
			//Create quad of two triangles from position and its neighbours
			//4 vertices of quads
			XMVECTOR v1 = XMVectorSet(m_terrainGeometry[x][y].position.x, m_terrainGeometry[x][y].position.y, m_terrainGeometry[x][y].position.z, 1);
			XMVECTOR v2 = XMVectorSet(m_terrainGeometry[x + 1][y].position.x, m_terrainGeometry[x + 1][y].position.y, m_terrainGeometry[x + 1][y].position.z, 1);
			XMVECTOR v3 = XMVectorSet(m_terrainGeometry[x + 1][y + 1].position.x, m_terrainGeometry[x + 1][y + 1].position.y, m_terrainGeometry[x + 1][y + 1].position.z, 1);
			XMVECTOR v4 = XMVectorSet(m_terrainGeometry[x][y + 1].position.x, m_terrainGeometry[x][y + 1].position.y, m_terrainGeometry[x][y + 1].position.z, 1);

			float dist;
			//Test for intersection on first triangle
			if (TriangleTests::Intersects(rayOrigin, rayDirection, v1, v2, v4, dist))
			{
				//Overwrite closest intersection if closer
			...
			//Test intersection on second triangle
			if (TriangleTests::Intersects(rayOrigin, rayDirection, v2, v3, v4, dist))
			{
				//Overwrite closest intersection if closer
				if (dist < closestDistance)
				{
	...
	XMFLOAT4 originFloat;
	XMFLOAT4 directionFloat;
	XMStoreFloat4(&originFloat, rayOrigin);
	XMStoreFloat4(&directionFloat, rayDirection);

	//Claculate point of intersection using ray and closest distance
	float mag = sqrt((directionFloat.x * directionFloat.x) + (directionFloat.y * directionFloat.y) + (directionFloat.z * directionFloat.z));

	float IntersectX = (originFloat.x + (directionFloat.x * (closestDistance / mag)) );
	float IntersectY = (originFloat.y + (directionFloat.y * (closestDistance / mag)) );
	float IntersectZ = (originFloat.z + (directionFloat.z * (closestDistance / mag)) );

	intersectPoint = XMVectorSet(IntersectX, IntersectY, IntersectZ, 1);
